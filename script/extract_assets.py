...
def extract_character_portraits():
  ...
  for item in current.fetchall():
    image_name = (item["name"].split("/")[-1]).split("_")[-1]
...
def extract_skill_icons():
  current.execute('SELECT n as name, h as hash from a WHERE n LIKE "outgame/skillicon/utx\_ico\_skill\_%" ESCAPE "\\"')
  for item in current.fetchall():
    image_name = (item["name"].split("/")[-1]).split("_")[-1]
    assets = UnityPy.load(os.path.join(asset_folder, "dat", item["hash"][:2], item["hash"]))
    for obj in assets.objects:
      if obj.type in [UnityPy.enums.ClassIDType.Texture2D, UnityPy.enums.ClassIDType.Sprite]:
        image = obj.read().image
        image.save(os.path.join(base_dir, "..\\public\\static\\image\\skill", image_name + ".png"))
